    ...
    <form method="POST" action="/admin/settings/update" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-400">Recommended: 250×250px, PNG or SVG (transparent bg)</p>
                </div>
                <!-- Favicon Upload -->
                <div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1">Logo Display Height (px)</label><input type="number" name="logo_height" value="<?= e($settings['logo_height'] ?? '40') ?>" min="20" max="300" step="2" class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm" placeholder="e.g. 40, 60, 100, 250"><p class="text-xs text-gray-400 mt-1">Logo height in header/footer (20–300px). Recommended upload: 250×250px. Default: 40px</p></div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <span id="pesapalTestResult" class="text-sm"></span>
                </div>
                <script>
                function testPesapalConnection() {
                    const btn = document.getElementById('pesapalTestBtn');
                    const result = document.getElementById('pesapalTestResult');
                    btn.disabled = true;
                    btn.innerHTML = '<svg class="animate-spin w-4 h-4" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg> Testing...';
                    result.textContent = '';
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <h3 class="font-medium mb-1 flex items-center gap-2"><i data-lucide="message-circle" class="w-5 h-5 text-amber-600"></i> Floating Chat Widget</h3>
            <p class="text-sm text-gray-500 mb-4">Show a floating WhatsApp and/or Telegram button on the bottom-right of every customer page.</p>
            <div class="space-y-5">
                <!-- WhatsApp -->
                <div class="border border-green-100 rounded-xl p-4 bg-green-50/30 space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="whatsapp_widget_enabled" value="1" <?= ($settings['whatsapp_widget_enabled'] ?? '') === '1' ? 'checked' : '' ?> class="sr-only peer">
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-green-100 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-600"></div>
                        </label>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">WhatsApp Number</label>
                        <input type="text" name="whatsapp_number" value="<?= e($settings['whatsapp_number'] ?? '') ?>" placeholder="e.g. 254712345678" class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm font-mono">
                        <p class="mt-1 text-xs text-gray-400">International format without + or spaces (e.g. 254712345678)</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Pre-filled Message (optional)</label>
                        <input type="text" name="whatsapp_message" value="<?= e($settings['whatsapp_message'] ?? '') ?>" placeholder="e.g. Hello! I'd like to inquire about..." class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm">
                    </div>
                </div>

                <!-- Telegram -->
                <div class="border border-sky-100 rounded-xl p-4 bg-sky-50/30 space-y-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2.5">
                            <svg class="w-5 h-5 text-sky-500" viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
                            <span class="text-sm font-semibold text-gray-800">Telegram</span>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="telegram_widget_enabled" value="1" <?= ($settings['telegram_widget_enabled'] ?? '') === '1' ? 'checked' : '' ?> class="sr-only peer">
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-sky-100 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-sky-500"></div>
                        </label>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Telegram Username or Phone</label>
                        <input type="text" name="telegram_number" value="<?= e($settings['telegram_number'] ?? '') ?>" placeholder="e.g. @yourshop or 254712345678" class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm font-mono">
                        <p class="mt-1 text-xs text-gray-400">Telegram username (e.g. @yourshop) or phone number</p>
                    </div>
                </div>
            </div>
        </div>
